...
window.ecoCart.admin.editingSetting = false; // Variable to store whether a setting is currently open for editing

// Upload variables
window.ecoCart.admin.fileSizeLimit = 5000000; // 5MB maximum file upload size
...
window.ecoCart.admin.ui = (function () {
    var
		...
		loadViewProducts = function () {
            ...
            document.querySelector("form#search-product-form>input").addEventListener("keyup", productSearchTyped);
            // </editor-fold>
        },
        ...
        loadViewSettings = function () {
            // <editor-fold defaultstate="collapsed" desc="loadViewSettings">            
            var section = document.createElement("section");
            section.setAttribute("id", "settings");
            var sectionHeader = document.createElement("header");
            sectionHeader.innerHTML = "Settings";
            section.appendChild(sectionHeader);
            
            // <editor-fold defaultstate="collapsed" desc="Settings">
            var div = document.createElement("div");
            div.setAttribute("id", "modifySettingsDiv");
            var settingsTable = getSettingsTable();
            div.appendChild(settingsTable);
            // </editor-fold>

            section.appendChild(div);
            window.ecoCart.mainElem.appendChild(section); // Add the settings section to the main element
        ...
        getSettingsTable = function () {
            // <editor-fold defaultstate="collapsed" desc="getSettingsTable">
            var settingsTable = document.createElement("table");
            settingsTable.setAttribute("id", "settingsTable");
            var headerTR = document.createElement("tr");
            headerTR.setAttribute("class", "heading");
            
            var headers = ["Setting", "Value", "Options"];
            ...
            settingsTable.appendChild(headerTR);
            
            // Populate settings
            window.ecoCart.xhr.sendRequest({
                url: window.ecoCart.apiURL + "settings",
                callback: {
                    load: function (xhr) {
                        var resp = JSON.parse(xhr.target.responseText).data;
                        ...
                        for (var key in resp) {
                            if (resp.hasOwnProperty(key)) { // Loop through the object
                                var obj = resp[key];
                                var tr = document.createElement("tr");
								tr.setAttribute("id", "setting-" + obj.id);
                                tr.dataset.setting = obj.id;
                                var td1 = document.createElement("td");
								td1.setAttribute("class", "name");
                                var td2 = document.createElement("td");
                                ...
                                var cleanName = getCleanSettingName(obj.name); // Present the setting name in a user-friendly way
                                td1.innerHTML = cleanName;
                                td2.innerHTML = obj.value;
                                
                                var editButton = document.createElement("button");
                                editButton.setAttribute("class", "edit-setting");
                                editButton.dataset.setting = obj.id;
                                editButton.dataset.settingName = obj.name;
                                editButton.innerHTML = "Edit";
                                td3.appendChild(editButton);
						...
						window.ecoCart.shared.addClassEvents("click", "edit-setting", editSettingClicked);
                        window.ecoCart.animations.hideLoading(); // Hide the loading screen
                    },
                    error: loadError
                }
        ...
        productSearchTyped = function (evt) {
            // <editor-fold defaultstate="collapsed" desc="productSearchTyped">			
			var searchInput = evt.target.value;
			if ((searchInput.length > 2) && window.ecoCart.shared.isCharacterKeyPress(evt)) { // If there's some search input and it's value
				var currProducts = document.getElementById("productsTable");
				var searchFieldset = document.getElementById("searchProduct");
				searchFieldset.removeChild(currProducts); // Remove the current table
				searchFieldset.appendChild(getProductsTable(searchInput)); // Append the new table
        ...
        editSettingClicked = function (evt) {
            // <editor-fold defaultstate="collapsed" desc="editSettingClicked">
            evt.preventDefault(); // Prevent default action, e.g. the form submitting and the page changing
            
            if (!window.ecoCart.admin.editingSetting) { // If a setting isn't currently open for editing
                window.ecoCart.admin.editingSetting = true;
                
                // Change the edit button for this setting into a save button
                evt.target.innerHTML = "Save";
                evt.target.setAttribute("class", "save-setting");
                window.ecoCart.shared.setClassDisabled("edit-setting", true);
                
                var id = evt.target.dataset.setting;
                var name = evt.target.dataset.settingName;
                var row = document.querySelector("table#settingsTable>tr#setting-" + id); // Get the corresponding row
                var nodes = row.childNodes; // nodes[1] - Value

                var input = document.createElement("input");
                input.setAttribute("id", "modSettingValue");
                if (window.ecoCart.shared.strEndsWith(name, "color")) {
                ...
                window.ecoCart.shared.addClassEvents("click", "save-setting", saveSettingClicked); // Add event listener for the save button
            }
            // </editor-fold>
        },
        saveSettingClicked = function (evt) {
            // <editor-fold defaultstate="collapsed" desc="saveSettingClicked">
            evt.preventDefault(); // Prevent default action, e.g. the form submitting and the page changing
            
            if (window.ecoCart.admin.editingSetting) { // If a setting is currently open for editing
                evt.target.disabled = true; // Disable the button so it can't be clicked
                var id = evt.target.dataset.setting;
                var name = evt.target.dataset.settingName;
                var value = document.getElementById("modSettingValue");
                
                var settingDetails = {
                    value: value.value
                };
                
                if (value.value.length > 0) { // If there's at least one character set for the setting's value
                    // Send this category update to the API to store it in the database
                    window.ecoCart.xhr.sendRequest({
                    ...
                    window.ecoCart.admin.editingSetting = false; // Another setting can now be edited
                }
            }
            // </editor-fold>
        },
            ...
            if (localStorage) { // If local storage is available
                if ((localStorage.getItem("viewToLoad") !== null) && (localStorage.getItem("viewToLoad") !== undefined)) {
                    ...
                    switch (view) {
                        ...
                        case "category":
                            window.ecoCart.admin.currentView = "category";
                            break;
                            
                        case "delivery-options":
                            window.ecoCart.admin.currentView = "delivery-options";
                            break;

                        case "products":
                            window.ecoCart.admin.currentView = "products";
                            break;
        ...
        getCleanSettingName = function (settingName) {
            // <editor-fold defaultstate="collapsed" desc="getCleanSettingName">
            var newSettingName = settingName;
            var finds = ["style-", "!id!", "!class!", "!attr!", /_/g]; // Mozilla. (2015). String.prototype.replace(). Retrieved 19/04/2015, from https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace
            var replaces = ["<span class=\"styleSetting\">style</span> ", "&#35;", "&#46;", "&#32;", "&#32;"];
            
            for (var i = 0; i < finds.length; i++) {
                newSettingName = newSettingName.replace(finds[i], replaces[i]);
            }
