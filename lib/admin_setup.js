...
window.ecoCart.admin.ui = (function () {
    var
        ...
        loadViewProduct = function (id) {
            ...
            var categorySelect = getCategorySelect("category", null, false);
            cell2.appendChild(categorySelect);
            row.appendChild(cell1);
            row.appendChild(cell2);
            formTable.appendChild(row);
		...
		getCategorySelect = function (selectID, defaultVal, parent) {
...
//							window.ecoCart.shared.addClassEvents("click", "edit-prod", editCatClicked);
							window.ecoCart.animations.hideLoading(); // Hide the loading screen
						},
						error: loadError
					}
		...
		editCatClicked = function (evt) {
            ...
            if (!window.ecoCart.admin.editingCat) { // If a category isn't currently open for editing
                ...
                nodes[1].appendChild(getCategorySelect("modCatParent", parentID, true));
