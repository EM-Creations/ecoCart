...
window.ecoCart.devAPIVersion = 2; // Dev API version to use
window.ecoCart.apiURL = "./api/" + window.ecoCart.apiVersion + "/";
window.ecoCart.devAPIURL = "./api/" + window.ecoCart.devAPIVersion + "/";
...
window.ecoCart.ui = (function () {
	var
				...
				for (var key in resp) {
					if (resp.hasOwnProperty(key)) { // Loop through the object, should only be one element
						...
						window.ecoCart.xhr.sendRequest({
							url: window.ecoCart.devAPIURL + "categories/parent/" + obj.cat,
							callback: {
		...
		setFooterDate = function () {
			...
			return function (xhr) {
				var resp = JSON.parse(xhr.target.responseText);
				document.querySelector("footer>span#date").innerHTML = resp.data;
			};
			// </editor-fold>
		},
		...
		loadViewHome = function () {
			...
			window.ecoCart.xhr.sendRequest({
				url: window.ecoCart.devAPIURL + "time/Y",
				callback: {
					load: setFooterDate(),
					error: loadError
				}
			...
			// Populate navigation
			window.ecoCart.xhr.sendRequest({
				url: window.ecoCart.devAPIURL + "categories/parent/0",
				callback: {
					load: populateLeftNav(),
					error: loadError
				}
		...
		populateNavCatHierarchy = function(startCat) {
			...
			window.ecoCart.xhr.sendRequest({
				url: window.ecoCart.devAPIURL + "categories/" + startCat,
				callback: {
					load: function (xhr) {
						var resp = JSON.parse(xhr.target.responseText).data;
						var main = document.querySelector("main");
