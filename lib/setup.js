...
window.ecoCart.ui = (function () {
	var
		...
		loadViewOrder = function () {
			...
			deliverySelect.setAttribute("id", "delivery-options");
			main.innerHTML = ""; // Clear the main element
			
			if (localStorage) { // If localStorage is available
				var basket = localStorage.getItem("ecoCartBasket");
				if ((basket !== null) && (basket.length > 0)) { // If the basket exists
					...
					orderSectionText.innerHTML = "<div id=\"orderSubTotal\">Order Sub-Total: &pound;" + totalPrice + "</span>";
					...
					orderSectionText.appendChild(detailsTable);
					orderSectionText.appendChild(completeButton);
					orderSectionText.appendChild(orderTotalDiv);
					...
					completeButton.addEventListener("click", completeOrderClicked); // Event listener to handle the complete order button being clicked
				...
				} else { // If the basket doesn't exist
					window.ecoCart.currentView = "basket"; // Redirect to basket
					loadView();
				}
			} else { // If local storage isn't available
				window.ecoCart.currentView = "basket"; // Redirect to basket
				loadView();
			}
			// </editor-fold>
		...
		completeOrderClicked = function () {
			// <editor-fold defaultstate="collapsed" desc="completeOrderClicked">
			// TODO: Complete order clicked
			console.log("completeOrderClicked called");
			// </editor-fold>
		},
