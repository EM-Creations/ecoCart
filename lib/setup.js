...
window.ecoCart.ui = (function () {
	var
		...
		addToBasketClicked = function (evt) {
			...
			customEvt.initCustomEvent("ecoCartAddToBasket", true, true, {id: evt.target.dataset.itemId, name: evt.target.dataset.itemName});
			...
			if (localStorage) { // If local storage is available
				...
				if (basket !== null) { // If the basket exists
					...
					if (!found) { // If we didn't find the item in the basket, we need to add it
						basket.push({id: evt.detail.id, name: evt.detail.name, qty: 1}); // Push the new item object onto the basket
					}
				} else { // If the basket doesn't exist, create it
					basket = [{id: evt.detail.id, name: evt.detail.name, qty: 1}];
				}
