	...
	var
		...
		populateLeftNav = function () {
			...
			return function (xhr) {
				...
				var catLinkClasses = document.getElementsByClassName("cat-link");
				for (var i = 0; i < catLinkClasses.length; i++) {
					catLinkClasses[i].addEventListener("click", categoryLinkClicked); // Event listener to handle category links being clicked
				}
