...
window.ecoCart.xhr = (function () {
    var
            ...
            sendRequest = function (req) {
                ...
                switch (req.method) {
					// <editor-fold defaultstate="collapsed" desc="Method switch">
                    case "POST":
                    case "PUT":
                        if (!req.noProcess || (req.noProcess === null)) { // If there's nothing set to stop processing
				...
				xhr.addEventListener("progress", window.ecoCart.shared.checkXHR); // Listener which checks progress of all XHR requests to see if any are erroring and handles them accordingly

                xhr.send(req.data);
                window.ecoCart.xhr.xhrLogger.log("sent " + req.method + " request to " + req.url); // Log that a request has been sent
                // </editor-fold>
            };
    return {
        "xhrLogger": xhrLogger,
